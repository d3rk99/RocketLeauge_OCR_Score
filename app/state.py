...
class StateManager:
    ...
    def reset_current_game_score(self) -> MatchState:
        return self.update(team_a_score=0, team_b_score=0, match_status="idle")

    def award_game(self, side: str) -> MatchState:
        current = self.snapshot()
        if side == "a":
            games = current.team_a_games + 1
            return self.update(team_a_games=games, team_a_score=0, team_b_score=0, current_game_number=current.current_game_number + 1)
        if side == "b":
            games = current.team_b_games + 1
            return self.update(team_b_games=games, team_a_score=0, team_b_score=0, current_game_number=current.current_game_number + 1)
